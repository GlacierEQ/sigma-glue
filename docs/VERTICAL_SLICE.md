...
- Optional local persistence writes atomic, restart-readable job snapshots, idempotency claims, and redacted NDJSON audit receipts.
- Persistence tests verify lifecycle recovery from disk and exclude raw plan paths and credential-like fields.
- 24/24 automated tests pass locally, including gateway routing, missing-gateway rejection, approval binding, raw-credential rejection, and restart-readable idempotency claims.
